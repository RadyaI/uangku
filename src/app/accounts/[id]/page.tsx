...
import {
  ArrowLeft, Edit2, Check, X, TrendingUp, TrendingDown,
  ArrowDownLeft, ArrowUpRight, Repeat2, Trash2, Plus, RefreshCw,
  AlertTriangle, Loader2, Wallet,
} from "lucide-react";
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <motion.div {...fade(0)} className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Sembunyikan tombol Transaksi untuk akun investasi karena punya panel sendiri */}
            {!isPortfolio && (
              <button
          ...
          <div className="flex items-start justify-between">
            <div>
              <p className="text-slate-400 text-sm font-medium mb-2">
                {isPortfolio ? "Total Modal Diinvestasikan" : "Saldo Saat Ini"}
              </p>
        ...
        {/* ── INVESTMENT ACCOUNT: Render InvestmentPanel ── */}
        {isPortfolio ? (
            ...
            >
              ...
              {modal === "editBalance" && (
                <>
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <button onClick={() => setModal("none")} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400"><X className="w-4 h-4" /></button>
                  </div>
                  ...
                  <button onClick={handleSaveBalance} className="w-full py-3 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2" style={{ background: accentColor }}>
              ...
              {modal === "editInfo" && (
                <>
                  ...
                  <div className="space-y-3">
                    <input type="text" placeholder="Nama Akun" value={infoForm.name} onChange={(e) => setInfoForm((p) => ({ ...p, name: e.target.value }))} className="w-full border border-slate-100 bg-slate-50 rounded-xl px-4 py-3 text-sm text-slate-700 font-semibold transition-all" autoFocus />
                    <select value={infoForm.type} onChange={(e) => setInfoForm((p) => ({ ...p, type: e.target.value as Account["type"] }))} className="w-full border border-slate-100 bg-slate-50 rounded-xl px-3 py-2.5 text-sm text-slate-700 font-medium transition-all">
                      <option value="bank">🏦 Bank</option>
                      <option value="mutual_fund">📊 Reksa Dana</option>
                      <option value="stock">📈 Saham</option>
                      <option value="cash">💵 Tunai</option>
                    </select>
                    <input type="text" placeholder="Mata Uang (IDR)" value={infoForm.currency} onChange={(e) => setInfoForm((p) => ({ ...p, currency: e.target.value.toUpperCase() }))} className="w-full border border-slate-100 bg-slate-50 rounded-xl px-3 py-2.5 text-sm text-slate-700 font-medium transition-all" />
                  </div>
                  <button onClick={handleSaveInfo} className="mt-4 w-full py-3 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2" style={{ background: accentColor }}>
              ...
              {(modal === "addTx" || modal === "editTx") && (
                <>
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <button onClick={() => { setModal("none"); setSelectedTx(null); }} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400"><X className="w-4 h-4" /></button>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {txForm.type === "transfer" && (
                      <div className="rounded-xl border border-amber-100 bg-amber-50 px-4 py-3">
                        ...
                        <input type="text" placeholder="Nominal biaya (Rp) — opsional" value={txForm.fee ? `Rp ${Number(txForm.fee.replace(/\D/g, "")).toLocaleString("id-ID")}` : ""} onChange={(e) => setTxForm((p) => ({ ...p, fee: e.target.value.replace(/\D/g, "") }))} className="w-full bg-transparent text-sm font-bold text-amber-700 placeholder:text-amber-300 focus:outline-none" />
                      </div>
                    )}
                    <input type="text" placeholder="Catatan (opsional)" value={txForm.note} onChange={(e) => setTxForm((p) => ({ ...p, note: e.target.value }))} className="w-full border border-slate-100 bg-slate-50 rounded-xl px-3 py-2.5 text-sm text-slate-700 font-medium transition-all" />
                  </div>
                  <button onClick={handleSaveTx} className="mt-4 w-full py-3 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2" style={{ background: accentColor }}>
